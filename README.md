...
- NuGet, to restore dependencies. Download from
  [http://nuget.org][NuGetInstall], or install via the Visual Studio Extension
  Gallery if using Visual Studio. Note: the build by default looks for nuget.exe
  in ./.nuget relative to this directory.
- [WiX][WiXId] v3.9.
...
nsis/Salt-Minion-$(DisplayVersion)-$(TargetPlatform)-Setup.exe.
...
The build will produce
wix/MinionMSI/bin/$(Configuration)/Salt-Minion-$(DisplayVersion)-$(TargetPlatform)-Setup.msi
and
wix/MinionEXE/bin/$(Configuration)/Salt-Minion-$(DisplayVersion)-$(TargetPlatform)-Setup.exe
.
